...
class simplex:
    ...
    def simplexFunction(self, A, c, b, HowToSolve, Phase, B=None, N=None):
        ...
        # REVIEW: 階層構造が深くなりすぎていて可読性が悪く，バグの温床につながるので，関数に分割，個別検証できるようにするとよい
        while True:
            # print(index_bases)
            ...
            if not np.linalg.matrix_rank(Base) == len(Base):
                ...
                if np.all(costFunction >= 0):
                    ...
                    if HowToSolve:
                        ...
                        col_index_in = np.linalg.solve(Base, Notbase[:, index_in : index_in + 1]).reshape(
                            Basis_num, 1
                        )  # 非基底列ベクトルの最小添字に対応する列
                    else:
                        # 最大係数規則で実行
                    ...
                    if np.all(col_index_in <= 0):
                        ...
                        # ピボット変換
                        index_notbases[index_in], index_bases[index_out] = (
                            index_bases[index_out],
                            index_notbases[index_in],
                        )
                        iter_num += 1  # 反復回数測定
...
class simplex_phase1:
    ...
    def phase1Function(self):
        ...
        subprobCoefficient = np.concatenate(
            (np.zeros(dimy_constMat).reshape(-1, 1), np.ones(dimx_constMat).reshape(-1, 1)), axis=0
        )  # 目的関数＝人工変数の和
        ...
        if value_phase1sol > 0:
            ...
            if np.all(np.isin(artificial_index, nonbaseIndex_phase1sol)):
                ...
                # REVIEW: 見てもらうときはこの手の試行錯誤プログラムは削除．ただし，後で振り返れるようにgitで履歴管理すべき
                ...
                for i in range(len(r)):
                    for j in range(len(s)):
                        ...
                        baseIndexForphase2[ridx[i]], nonbaseIndexForphase2[sidx[j]] = (
                            nonbaseIndexForphase2[sidx[j]],
                            baseIndexForphase2[ridx[i]],
                        )
                        # 入れ替え後の基底行列の正則性判定
...
class simplex_phase2:
    def otimization(self, c, A, b):
        ...
        if solphase1.phase1Function() is None:
            ...
            # 第2段階を解く
            solphase2 = simplex()
            sol = solphase2.simplexFunction(
                self.constraintMatrix, self.objcoefficcient, self.rightVector, False, True, initBaseIdx, initNonbaseIdx
            )
            return sol
